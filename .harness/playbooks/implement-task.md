...
9. `before-edit` checklist를 적용한다.
10. 작업 상태를 `IN_PROGRESS`로 갱신한다.
11. Scope 안에서만 구현 또는 문서 작업을 수행한다.
12. Validation을 실행한다.
13. 검증 성공 시 `update-task-state` skill로 DONE 처리한다.
14. `summarize-run` skill 형식으로 보고한다.
